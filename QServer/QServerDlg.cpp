...
CQServerDlg::CQServerDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_QSERVER_DIALOG, pParent)
	, m_showMsg(_T(""))
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CQServerDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CQServerDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_StartBtn, &CQServerDlg::OnBnClickedStartbtn)
...
BOOL CQServerDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	//修改主对话框的背景
	m_brBg.DeleteObject();					//释放绑定的对象内存
	m_bmBg.DeleteObject();
	m_bmBg.LoadBitmap(IDB_bg);
	m_brBg.CreatePatternBrush(&m_bmBg);		//用位图指定的模式初始化画刷
...
//显示。
HCURSOR CQServerDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

HBRUSH CQServerDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//用户退出，群发退出消息
void CQServerDlg::ClosePlayer(CMySocket* from)
{
	int i, msg_len;
	TCHAR out_msg[200];
	...
	m_csList.Lock();
	for (i = 0; i < m_list.GetItemCount(); i++)//列表控件项的数目
	{
		if (m_list.GetItemData(i) == (DWORD)from)//GetItemData通过nItem来获取与指定项相关的32位应用指定的值。
		{
			delete from;
			nbuf[0] = 0x41;//第一个字节表示类型，加入，退出，私聊群发
			m_list.GetItemText(i, 0, nbuf + 1, 100); //i号索引内容读到nbuf缓冲区，长度为100
...
//追加消息体
void CQServerDlg::Append(TCHAR* msg)
{
	//读取消息框中所有的消息
	m_showMsg += msg;
	UpdateData(FALSE);
...
void CQServerDlg::ParserPkt(CMySocket* from)
{
	...
	if (SendBuff[0] == 0x11)
	{
		...
		from->GetPeerName(ipaddr, port);       //获取套接字被连接的对等套接字的地址。
		//与SOCKET通信的用户的称谓
	...
	else if (SendBuff[0] == 0x51)	//根据用户名转发了
	{
		...
		memset(pName, 0, 100);		//memset初始化pName
		memset(bName, 0, 100);
	...
	else	//向所有用户转发消息	
	{
		...
		memcpy(ShowBuff, SendBuff + 1, 4096);  //把SendBuff+1开始的内容拷贝到ShowBuff中
...
//开启服务器按钮，创建Socket
void CQServerDlg::OnBnClickedStartbtn()
{
	// TODO: 在此添加控件通知处理程序代码
	m_socket.AttachCWnd(this);
	BOOL isTrue = m_socket.Create(0x8123, SOCK_STREAM);  //TCP协议
	if (isTrue)
	{
		m_socket.Listen();
